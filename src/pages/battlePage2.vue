<template>
  <div class="">
    <div class="carousel" v-if="!showAll">
      <Carousel :autoplay="true" dots="inside" :radius-dot="true" :loop="true" arrow="never" :height="800"
        :autoplay-speed="4000">
    ...
    <div v-show="showAll">
      <Row class="battle-top" type="flex" align="middle" :style="{ 'display': isControlPanelExpand ? '' : 'none' }">
        ...
        <Row style="padding:1rem 5rem;color:#fff;font-size:2rem;">
          <Col span="12" style="background-color:green;padding:1rem 0;border-radius:20px 0 0 20px;"
            @click.native="changeScoreStatus(1)">
          加分
          </Col>
          <Col span="12" style="background-color:red;padding:1rem 0;border-radius:0 20px 20px 0;"
            @click.native="changeScoreStatus(-1)">
          减分
          </Col>
        </Row>
      ...
      <div class="spacer" :class="[scoreStatus==1?'green':'red']">
        <Row>
          ...
          <Col span="4">
          <Button type="primary" shape="circle" size="large" icon="arrow-shrink" v-if="isControlPanelExpand"
            @click="setControlPanelVisable(false)"></Button>
          <Button type="primary" shape="circle" size="large" icon="arrow-expand" v-if="!isControlPanelExpand"
            @click="setControlPanelVisable(true)"></Button>
          </Col>
          <Col span="10">
          比赛已开始：{{duration}}
          </Col>
        ...
        <transition name="fade">
          ...
          <Row style="" v-if="showMembers">
            <transition-group name="flip-list" tag="ul">
              <div class="" style="color:#fff;margin-top:0.5rem;" v-for="(team,index) in teamsOrderByScore"
                :key="team._id">
                <team-bar :team="team" :rank="index+1" :addScoreToGroup="addGroupScore"
                  :members="groupMembers(team._id)" :addScore="addScore" :scoreStatus="scoreStatus" :key="team._id">
                </team-bar>
              </div>
            </transition-group>
      ...
      <Modal v-model="opRandomNumberModal" width="80%" class-name="vertical-center-modal">
        <p slot="footer"></p>
        <random-member-panel :members="members" :show="showPanel" :playMusic="playMusic"
          :addMemberPickedCount="addMemberPickedCountMethod">
        </random-member-panel>
      </Modal>

      <Modal v-model="showMemberBoard" width="85%" :closable="false" class-name="vertical-center-modal">
        <p slot="footer"></p>
        <member-board v-if="selectedMember" :member="selectedMember" :addScoreToMember="addScore" :playMusic="playMusic"
          :removeCard="removeCard">
        </member-board>
      </Modal>

      <Modal v-model="showWinnerModal" width="80%" :closable="false" :mask-closable="false" ok-text="新的比赛"
      ...
      <Modal v-model="showMVPModal" width="80%" class-name="vertical-center-modal">
        <div style="text-align: center;font-size:5rem; color:#5cadff;margin-top:20px;">
          王者
        </div>
        <div class="panel" style="padding:2rem;text-align:center">
      ...
      <Modal v-model="showRandomEventModal" width="80%" class-name="vertical-center-modal">
        <RandomEventPanel :randomEvents="getRandomEvents" :playMusic="playRandomEventMusic" v-if="showRandomEventModal">
        </RandomEventPanel>
        <p slot="footer"></p>
      </Modal>
      <Modal v-model="showCardBoard" width="85%" :closable="false" class-name="vertical-center-modal">
        <p slot="footer"></p>
        <card-board :cards="selectedTheme.randomCards"></card-board>
      </Modal>

      <audio ref="audioUseCard"
        :src="selectedTheme.useCardSound ? selectedTheme.useCardSound.URL : '/static/audio/Events/useCard.m4a'"
        preload="auto" style="display: none;"></audio>
      <audio ref="audioGetScore"
        :src="selectedTheme.getScoreSound ? selectedTheme.getScoreSound.URL : '/static/audio/Events/get.m4a'"
        preload="auto" style="display: none;"></audio>
      <audio ref="audioRandomEvent"
        :src="selectedTheme.randomEventSound ? selectedTheme.randomEventSound.URL : '/static/audio/Events/randomEvent.wav'"
        preload="auto" style="display: none;"></audio>
      <audio ref="audioRandomPeople"
        :src="selectedTheme.randomPeopleSound ? selectedTheme.randomPeopleSound.URL : '/static/audio/Events/randomPeople.wav'"
        preload="auto" style="display: none;"></audio>
      <audio ref="audioLostScore"
        :src="selectedTheme.lostScoreSound ? selectedTheme.lostScoreSound.URL : '/static/audio/Events/lost.m4a'"
        preload="auto" style="display: none;"></audio>
      <audio ref="audioLoadBattle"
        :src="selectedTheme.loadBattleSound ? selectedTheme.loadBattleSound.URL : '/static/audio/Events/loadBattle.m4a'"
        preload="auto" style="display: none;"></audio>

      <audio v-for="(rEvent, index) of getRandomEvents" :key="index" :ref="rEvent._id"
        :src="rEvent.audioFile ? rEvent.audioFile.URL :'/static/audio/Events/randomEvent.wav'" preload="auto"
  ...
  export default {
    ...
    data() {
      return {
        ...
        carouselItems: [
          'https://qn.file.classmaster.cn/Carousel1.jpg-1080p',
          'https://qn.file.classmaster.cn/Carousel3.jpg-1080p',
          'https://qn.file.classmaster.cn/Carousel2.jpg-1080p',
          'https://qn.file.classmaster.cn/Carousel4.jpg-1080p',
        ],
      };
    },
    computed: {
